  ...
  sio.on('connection', function (socket) {
    ...
    game.on('start', function () {
      ...
      socket.on('sync time', function (t) {
        socket.emit('sync time 2', Date.now() - t)
      })

      socket.on('ready', function () {
        socket.emit('game start', game.startTime)
      })

      socket.emit('sync time 1', Date.now())
    })
    ...
    game.on('reset', function (user) {
      game.resetTime = Date.now()
      socket.emit('game reset', user, game.resetTime)
    })
  })
